...
Func OpenBS($bRestart = False)
	...
	SetLog(getLocaleString("logStartBS"), $COLOR_GREEN)
	...
	$hTimer = TimerInit()
	SetLog(getLocaleString("logWaitBSstart"), $COLOR_GREEN)
	While IsArray(ControlGetPos($Title, "_ctl.Window", "[CLASS:BlueStacksApp; INSTANCE:1]")) = False
		...
		If $iCount > 240 Then ; if no BS position returned in 4 minutes, BS/PC has major issue so exit
			SetLog(getLocaleString("logBSLoadError"), $COLOR_RED)
			SetLog(getLocaleString("logBSLoadError2") & Round(TimerDiff($hTimer) / 1000, 2) & getLocaleString("logBSLoadError3"), $COLOR_RED)
	...
	If IsArray(ControlGetPos($Title, "_ctl.Window", "[CLASS:BlueStacksApp; INSTANCE:1]")) Then
		SetLog(getLocaleString("logBSLoaded") & Round(TimerDiff($hTimer) / 1000, 2) & getLocaleString("logBSLoaded2"), $COLOR_GREEN)
...
Func waitMainScreenMini()
	...
	SetLog(getLocaleString("logBSRestartMSC"), $COLOR_BLUE)
	For $i = 0 To 60 ;30*2000 = 1 Minutes
		...
		Else
			SetLog(getLocaleString("logCocMainWindowLoad") & Round(TimerDiff($hTimer) / 1000, 2) & getLocaleString("logCocMainWindowLoad2"), $COLOR_GREEN)
			Return
		EndIf
		_StatusUpdateTime($hTimer)
