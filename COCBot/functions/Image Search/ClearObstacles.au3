...
Func ClearObstacles()
	...
	SetLog(getLocaleString("logClearObstacles"), $COLOR_BLUE)
	...
	If $FreeBuilder < 1 Then
		SetLog(getLocaleString("logNoBuilders"), $COLOR_RED)
		Return
	EndIf
				...
				For $j = 1 To UBound($expRet) - 1 Step 2
					...
					If isInsideBigDiamondXY($ObstacleX, $ObstacleY) Then 					
						...
						If _ColorCheck(_GetPixelColor(432, 592), Hex(0xE82CD0, 6), 20) Then ; Check Elix
							SetLog(getLocaleString("logObstacleFound"), $COLOR_GREEN)
							If _Sleep(500) Then Return
	...
	$ObsFolder += 1
	SetLog(getLocaleString("logNoObstacleFound"), $COLOR_RED)
