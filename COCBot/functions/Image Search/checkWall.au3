...
;Additional Wall Finding
			...
			Else
				...
				For $j = 1 To UBound($expRet) - 1 Step 2
					...
					If IsInsideDiamondXY($WallX, $WallY) Then
						If $debugSetlog = 1 Then
							SetLog(getLocaleString("logWallLevel") & $icmbWalls + 4 & " • Position: [" & $WallX & "," & $WallY & "]" & getLocaleString("logWallLevelVerify"), $COLOR_PURPLE)
						Else
							SetLog(getLocaleString("logWallLevel") & $icmbWalls + 4 & getLocaleString("logWallLevelVerify"), $COLOR_GREEN)
						EndIf
						GemClick($WallX, $WallY,1,0,"#0122")
	...
	SetLog(getLocaleString("logRearrangeWalls"), $COLOR_RED)
	;$NoMoreWalls = 1
	Return False
