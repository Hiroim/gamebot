...
Func ReArm()
	...
	SetLog(getLocaleString("logCheckRearm"), $COLOR_BLUE)
	...
	If IsArray($RearmPixel) Then
		...
		If isGemOpen(True) = True Then
			Setlog(getLocaleString("logRearmInsufficientLoot"), $COLOR_RED)
			Click(585, 252,1,0,"#0227")  ; Click close gem window "X"
			If _Sleep($iDelayReArm5) Then Return
		Else
			SetLog(getLocaleString("logRearmed"), $COLOR_GREEN)
			If _Sleep($iDelayReArm5) Then Return
		EndIf
	Endif
	...
	If IsArray($XbowPixel) Then
		...
		If _Sleep($iDelayReArm4) Then Return
		If isGemOpen(True) = True Then
			Setlog(getLocaleString("logRearmInsufficientLootXbow"), $COLOR_RED)
			Click(585, 252,1,0,"#0230")  ; Click close gem window "X"
			If _Sleep($iDelayReArm5) Then Return
		Else
			SetLog(getLocaleString("logReloadedXbows"), $COLOR_GREEN)
			If _Sleep($iDelayReArm5) Then Return
		EndIf
	EndIf
	...
	If IsArray($InfernoPixel) Then
		...
		If _Sleep($iDelayReArm4) Then Return
		If isGemOpen(True) = True Then
			Setlog(getLocaleString("logRearmInsufficientLootInferno"), $COLOR_RED)
			Click(585, 252,1,0,"#0233")  ; Click close gem window "X"
			If _Sleep($iDelayReArm5) Then Return
		Else
			SetLog(getLocaleString("logReloadedInfernos"), $COLOR_GREEN)
			If _Sleep($iDelayReArm5) Then Return
		EndIf
	EndIf
